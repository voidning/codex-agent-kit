Please install this repository's on-demand multi-agent workflow as my personal global Codex configuration on this computer. Perform the setup and validation, rather than only providing instructions.
...
3. Merge the workflow into the personal global `AGENTS.md`, preserving unrelated user instructions. Resolve conflicting delegation rules in favor of this explicitly requested workflow and explain the changes.
...
7. Validate TOML syntax, role names, models, reasoning levels, the concurrency limit, and workflow consistency. Verify client loading when possible. Do not start paid subagent calls merely to test installation.
